...
{
	// S_IRUGO is 0444
	mymounts_file = proc_create("mymounts", 0444, NULL, &mymounts_ops);
	if (!mymounts_file) {
		pr_err("/proc/mymounts: proc_create failed\n");
		return -EFAULT;
	}
